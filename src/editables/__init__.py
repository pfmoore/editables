...
class EditableProject:
    ...
    def map_namespace(self, name, target):
        abs_target = self.make_absolute(target)
        if abs_target.is_dir():
            self.namespaces[name].append(abs_target)
        else:
            raise EditableException(f"{target} is not a valid namespace directory")
    ...
    def files(self):
        ...
        if self.redirections or self.namespaces:
    ...
    def bootstrap_file(self):
        ...
        for name, path in self.namespaces.items():
            bootstrap.append(f"F.add_namespace_path({name!r}, {str(path)!r})")
        return "\n".join(bootstrap)
